...
def pipe_run(filelist,outdir,size=None,center=None,sim=False):
    ...
    ref = filelist[0]
    print 'Cross-correlating images to ref=%s' % ref

    outlist = []
    for filename in filelist:
        xcorr_im = xcorr(ref,filename,size=size,center=center)
        shiftx, shifty = find_shift(xcorr_im)
        print '  %s\t(%i, %i)' % (filename,shiftx,shifty)

        outfile = os.path.splitext(filename)[0] + '.shft.fits'
        outfile = os.path.join(outdir,os.path.basename(outfile))


        if not sim:
            image2, header = pyfits.getdata(filename, header=True)
            image2 = shift(image2, (shifty,shiftx), cval=np.nan)
            header['SHFT_REF'] = (ref, 'Reference image of shift')
